...
\setbeamertemplate{footline}{
  ...
  \begin{beamercolorbox}[wd=.3333\paperwidth,ht=2.25ex,dp=1ex,center]{author in head/foot}%
    \usebeamerfont{author in head/foot} \color{white} Yichao Jin (UTD)
  \end{beamercolorbox}%
  \begin{beamercolorbox}[wd=.3334\paperwidth,ht=2.25ex,dp=1ex,center]{title in head/foot}%
    \usebeamerfont{title in head/foot} \color{white} Vaccination Timing DCE
  \end{beamercolorbox}%
    ...
    \begin{tikzpicture}[remember picture, overlay]
        ...
        \node[anchor=center, yshift=0.5cm] at (current page.center) {
            \begin{minipage}{0.9\paperwidth}
                \centering
                {\Huge \bfseries \color{utdgreen} Intertemporal Choice in Vaccination Timing} \\[0.4cm]
                {\Large \color{utdorange} A Discrete Choice Experiment Approach} \\[0.8cm]
                \textcolor{utdgreen}{\rule{0.6\textwidth}{1.2pt}} \\[0.8cm]
                {\large \scshape Dissertation Defense: Evidence from Wuhan, China}
            \end{minipage}
        };
        ...
        \node[anchor=south west, xshift=1cm, yshift=1.2cm] at (current page.south west) {
            \begin{tabular}{l}
                {\large \textbf{Yichao Jin}} \\
                {\footnotesize Candidate for PhD in Public Policy and Political Economy} \\
        ...
        \node[anchor=south east, xshift=-1cm, yshift=1.2cm] at (current page.south east) {
            ...
            \begin{tabular}{rl}
                ...
                \textcolor{utdgreen}{Dohyeong Kim} & (Chair) \\
                Richard Scotch & \\
                Soyoung Kwon & \\
                Khai Chiong &
            \end{tabular}
...
% --- Slide 9: Results I: Model Performance ---
\begin{frame}{Results I: Model Performance and Fit}
    \begin{itemize}
        \item \textbf{Comparison:} Comparison of Multinomial Logit (MNL) vs. Mixed Logit (MXL).
        \item \textbf{Statistical Superiority:} MXL significantly outperforms MNL ($p < 0.001$).
        \item \textbf{Conclusion:} Significant preference heterogeneity exists; modeling it is crucial for accurate valuation.
    \end{itemize}
    \begin{table}
        \centering
        \small
        \begin{tabular}{lcc}
            \toprule
            Metric & Multinomial Logit (MNL) & Mixed Logit (MXL) \\
            \midrule
            Log-Likelihood & -6096.612 & \textbf{-5615.522} \\
            AIC & 12207.225 & \textbf{11245.044} \\
            Obs. (N $\times$ T) & 6,162 & 6,162 \\
            \bottomrule
        \end{tabular}
    \end{table}
\end{frame}
...
% --- Slide 11: Results II: Quantifying Present Bias (Kappa) ---
\begin{frame}{Results II: Quantifying Present Bias}
    \begin{columns}[T]
        \column{0.5\textwidth}
        \begin{block}{Parameter Estimates}
            \begin{itemize}
                \item $\hat{\delta} = 0.160$ (Baseline Discounting)
                \item $\hat{\kappa} = 0.225$ (Present Bias Factor)
            \end{itemize}
        \end{block}
        \column{0.5\textwidth}
        \begin{alertblock}{The Meaning of $\kappa$}
            \begin{itemize}
                \item $\kappa < 1$ confirms \textbf{Present Bias}.
                \item Significant drop in perceived value for even a 1-month delay.
            \end{itemize}
        \end{alertblock}
    \end{columns}
    \centering
    \vspace{0.5cm}
    \textit{Conclusion: Vaccination delay is not just a logistical hurdle, but a behavioral barrier.}
\end{frame}
